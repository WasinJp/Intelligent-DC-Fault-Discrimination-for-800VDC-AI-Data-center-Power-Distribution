...
```
PI tuning for a capacitor plant with an ideal inner loop: `K_i = C_out (2π f_v)²`, `K_p = 2 ζ_v √(K_i C_out)`, ζ_v = 0.7. With the storage on the output (C_out up to 5 F) the voltage loop is slow by design (f_v 20 Hz–2 kHz); the current limit still engages within τ_i. The converter is off after the input UVLO trips (§4.10). **Numerical note (v0.4.4, from check (i)).** The anti-windup rule — integrate ξ_v only while the final clamped command equals i_ref — has a discontinuous right-hand side, and with the power bound of §4.7 following τ_r behind, the loop *slides* along that boundary after a demand ramp. An explicit fixed-step scheme chatters across the boundary and converges to the sliding (Filippov) solution: the core's result is step-independent from 50 ns to 10 µs (check (d)) and is reproduced by independent explicit solvers (RK45, RK23) to three decimals. Implicit solvers are not reliable here (Radau gave P_cmd 1.19 p.u. against 0.93; Simulink's ode23t diverged). Any re-implementation must integrate ξ_v, P_cmd and i_co explicitly — which is also what a digital controller does. Structure follows the aggregated buck-converter equivalent with cascaded voltage–current control of arXiv:2604.06624; the per-stage loss structure follows arXiv:2608.17925.
...
RK4 at 10 µs is stable for τ_c ≥ 16 µs and τ_i ≥ 10 µs (dt/τ ≤ 1) and resolves the fastest healthy resonance (5 kHz) at 20 steps per period.

**Held faults in the coarse tiers (v0.4.4).** The statement above covers the healthy network. A fault branch has τ_f = L_f / (R_f + R_esr) down to 0.05 µs, and RK4 is unstable for dt/τ_f > 2.785. While faults cleared at 5 ms — the end of the event window — no coarse step ever saw one. `FAULT_MODE = "held"` (0.4.3) keeps the branch on through the 2 µs and 10 µs tiers, where its current diverged and the causal anti-alias filter carried the NaN to the end of the record. Rule: in a step with dt ≥ 1.5 µs **and** dt > 0.5·τ_f, the branch current is set to the slow limit of its own equation — `L_f di_f/dt = v_node − R_f i_f` with the inductor term dropped, solved together with the node's ESR drop: `i_f = (v_C + R_esr (i_L − i_Lin)) / (R_f + R_esr)`, and likewise `i_f48 = (v_out + R_out_esr (i_co − i_pol)) / (R_f48 + R_out_esr)`. Ten milliseconds after inception the dropped term is below 10⁻³ of the branch voltage for every τ_f that triggers the rule. The 1.5 µs floor sits between the grids that integrate a fault *front* (the 50 ns event window; the 1 µs uniform grid of the check-(i) references) and the coarse tiers, so no Layer-1/2 observable and no check-(i) reference changes. Validated by check (j). A 7.6 s history costs ~0.75 M steps. Per event: ~0.7–1.5 s.
...
| (j) | **held fault through the coarse tiers** (v0.4.4): production tiers with the quasi-static fault branch vs an all-50 ns reference that integrates the branch everywhere; high_z at baseline and at its stiffest (τ_f 0.05 µs), resistive_pp at 1 Ω, high_z_48 at 100 mΩ / 0.1 µH; i_L, v_bus, i_rack, v_out over +5 to +60 ms; every sample finite; and the same schedule *without* the rule must go non-finite | ≤ 10⁻³ | 5 × 10⁻⁵ |
| (i) | **Simscape Electrical cross-check** (independent implementation): single shelf, variable-step; step, high_z, high_z_48, bolted_48, each with smoothing off and on (8 runs); feeder and rack waveforms on the 1 µs reference grid | (1) discontinuity-excluded normalised RMS ≤ 1 % on i_L, v_bus, i_rack, v_out, exclusions ±5 µs around adjacent-sample jumps > 0.1 p.u. in the reference (the POL-UVLO edge in bolted_48; 5 µs is 20× the reference-grid jitter and 1/20 of t_uvlo48); (2) every such edge matched in the Simscape run within 5 µs and 20 % in size; (3) raw RMS reported alongside. Registered 2026-09-17 (window narrowed from ±50 µs the same day, before any Simscape result). | **PASS, 8 of 8** (R2026a, 2026-09-18, criterion unchanged). Worst excluded-RMS per run, all on i_L: `step` 2.9 × 10⁻⁴, `step_sm` 1.9 × 10⁻⁴, `high_z` and `high_z_sm` 2.7 × 10⁻⁴ (source current limit and ORing diode act), `high_z_48` 8.8 × 10⁻⁴, `high_z_48_sm` 5.3 × 10⁻⁴, `bolted_48` 4.2 × 10⁻³, `bolted_48_sm` 8.3 × 10⁻⁴; v_bus, i_rack, v_out ≤ 5.9 × 10⁻⁴ everywhere; POL-UVLO edge matched at 0.00 µs and within 0.2 % in size in both bolted runs; pre-event window flat to 1.5 × 10⁻⁷ p.u. History: the first full run passed 6 of 8; `step_sm` (0.19) and `high_z_48_sm` (0.70) failed because Simulink's implicit solver cannot integrate the discontinuous anti-windup rule — the logged ξ_v moved against its own derivative (9 × 10⁻⁴ → −1913 with dξ_v/dt ≥ 0). The Simscape control states are now advanced by an explicit RK4 at 1 µs (`ctrl_d`), the network stays on ode23t; the prediction registered before the re-run (both pass, the other six within a factor of 3 of their previous value) held. See the §4.8 note. |

`scripts/validate_model.py` runs (a)–(h) and (j) and prints ALL PASS. (i) is the independent-implementation credential, built from this document and run with `simscape/crosscheck.m` (one command; log in `simscape/crosscheck_log.txt`).
...
| OPEN-11 | sanity-gate censoring of large segments; Gate-2 99th-percentile row | **percentile rows closed, v0.4.4** (v1-large, 800 V high-Z gray zone 72.0 / 57.0 / 36.5 % at 0 / 0.26 / 2.52 % false trips for max / 99.9th / 99th percentile); gate censoring: still report the accepted P_rated marginal |
...
| 0.4.4 | 2026-09-18 | **Held faults in the coarse tiers.** Found while testing sharded generation: post-event slow streams of held faults were non-finite in dataset v1-large — 180/200 high_z, 30/200 resistive_pp, 26/200 high_z_48 (feeder and rack), no bolted, arc or benign event (`scripts/check_postrecord.py`; predicted from the sweep ranges at 92 / 17 / 9 % before the file was checked). Cause and rule in §8.1; check (j) added. A first version of the rule with a 1 µs floor moved two of the eight check-(i) references (they integrate the fault front on a uniform 1 µs grid) and was caught by the regression test; floor set to 1.5 µs. Regression on 18 regenerated events: all fast streams, both feature tables and all eight check-(i) references bit-identical to 0.4.3; slow streams change only in held-fault events; no non-finite sample. **Consequence:** every Layer-1/2 result on v1-large stands (features never read past +1 ms); the v1-large post-event record is invalid for Layer 3 and the dataset must be regenerated before that study. **Full-scale confirmation (2026-09-18):** v1-large regenerated on 0.4.4 in four shards (`data/events_v1large044_s*.h5`): `check_postrecord` clean on all nine labels (was 180 / 30 / 26 events non-finite), node study identical to the 0.4.3 table in every digit, `compare_feature_tables` REGRESSION PASS (both node tables bit-identical, 18 603 rows each). Also: sharded generation (`--shard i/n`, bit-identical to a monolithic run), `--benign N`, multi-file datasets (`iter_events`), node-cache guard against a table from another dataset, operating study (`scripts/run_operating_study.py`: Gate-2 percentile rows — OPEN-11 closed; two-node operating curve with held-out calibration; decision-window table). Simscape harness rewritten around `crosscheck.m` (ports detected, not assumed; 0.2 µs sense lags break the Simulink–Simscape algebraic loop; default block parasitics kept and compensated). |
